...
public class StartupLogic {
    ...
    public void fromLostriver(File projectDir, String domain, String serviceName) {
        String packageName = buildPackageName(domain);
        new NewProjectCreator("lostriver", projectDir, original -> {
            String filtered = original;
            filtered = replace(filtered, buildProjectDirPureName(projectDir), Srl.initUncap(serviceName)); // e.g. lastaflute-example-lostriver
            filtered = replace(filtered, "lastaflute-example-lostriver", Srl.initUncap(serviceName)); // just in case
            ...
            filtered = replace(filtered, "Lostriver", Srl.initCap(serviceName));
            filtered = replace(filtered, "lostriver", Srl.initUncap(serviceName));
            filtered = replace(filtered, "new JettyBoot(8156, ", "new JettyBoot(9001, ");
            filtered = replace(filtered, "new TomcatBoot(8156, ", "new TomcatBoot(9001, ");
            return filtered;
        }).newProject();
    }
