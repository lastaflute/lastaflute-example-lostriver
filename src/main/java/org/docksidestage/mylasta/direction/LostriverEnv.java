...
public interface LostriverEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/lostriver */
    ...
    /** The key of the configuration. e.g. lostriver-support@annie.example.com */
    String MAIL_ADDRESS_SUPPORT = "mail.address.support";

    /** The key of the configuration. e.g. org.h2.Driver */
    String JDBC_DRIVER = "jdbc.driver";

    /** The key of the configuration. e.g. jdbc:h2:mem:lostriverdb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. lostriverdb */
    ...
    /** The key of the configuration. e.g. localhost:8156/lostriver */
     ...
     * The value is, e.g. /tmp/lastaflute/lostriver <br>
     ...
     * The value is, e.g. lostriver-support@annie.example.com <br>
     * comment: Mail Address for Lostriver Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * The value is, e.g. jdbc:h2:mem:lostriverdb <br>
     * comment: The URL of database connection for JDBC: *dummy database
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
    String getJdbcUrl();
     ...
     * The value is, e.g. lostriverdb <br>
     ...
     * The value is, e.g. localhost:8156/lostriver <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements LostriverEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(LostriverEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(LostriverEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(LostriverEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(LostriverEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(LostriverEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(LostriverEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(LostriverEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(LostriverEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(LostriverEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(LostriverEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(LostriverEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(LostriverEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(LostriverEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(LostriverEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(LostriverEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(LostriverEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(LostriverEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(LostriverEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(LostriverEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(LostriverEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(LostriverEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(LostriverEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(LostriverEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(LostriverEnv.SERVER_DOMAIN);
        }
    }
}
